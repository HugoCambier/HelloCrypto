...
function renderRunParamsTab() {
  ...
  const items = [
    ...
    ['Seuil maintien', fmtVal(p.hold_threshold)],
    ['Taille panier',  fmtVal(p.top_n)],
...
function _buildRecapMarkdown() {
  ...
  const lines = [
    ...
    `- Seuils : achat ${p.buy_threshold ?? '—'} · maintien ${p.hold_threshold ?? '—'} · panier ${p.top_n ?? '—'}`,
...
async function startBacktest() {
  ...
  const body = {
    ...
    hold_threshold: Number(document.getElementById('bt-hold-thr').value),
